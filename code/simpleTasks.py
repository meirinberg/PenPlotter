...
def runUIMode(filenames, freeDrawMode, UIMode):
    i = 0
    line1 = filenames[i]
    line2 = filenames[i+1]
    while UIMode:
...
def runFreeDrawMode():
    motor_radial = StepperMotor(1, 0b00100000, 3000, 5000, True)
...
# #                    datay.append('PD')
   ...
   for t in range(len(datax)):
      ...
      #print(t)
      X = [datax[t],datay[t]]
      temp = NewtonRaphson(lambda theta: g(X, theta), dg_dtheta, guess, .001)
...
#               f2 = temp[1]*math.sin(temp[0])/alpha
      ...
      datax[t] = int((temp[0])*motor_theta.NEMA_FULL_ROTATION / (2*math.pi))
      datay[t] = int((temp[1])*motor_radial.OTHER_FULL_ROTATION*100 / (2*math.pi))
...
#       else:
#          light.off()
   #datax = [200,400,400,200,200]
   #datay = [6000,6000,12000,12000,6000]
   i = 0
   pen.down()
   while i < len(datax):
      #time.sleep(1)
      ...
      line1 = "Processing Data:"
      line2 = "Point " + str(i+1) + " of " + str(len(datax))
      display.lcd_string(line1, display.LCD_LINE_1)
      display.lcd_string(line2, display.LCD_LINE_2)
...
def main():
   ...
   ui_mode = cotask.Task(runUIMode(filenames, freeDrawMode, UIMode), name = "UI Mode", priority = 1, period = 1500, profile = True, trace = False)
   free_draw = cotask.Task(runFreeDrawMode, name = "Free Draw Mode", priority = 1, period = 50, profile = True, trace = False)
   file_draw = cotask.Task(runFileDrawMode, name = "Free Draw Mode", priority = 1, period = 50, profile = True, trace = False)
   ...
   while True:
       ...
       if UIMode:
          cotask.task_list.append(ui_mode)
       if not UIMode:
           if not runFreeDrawMode():
