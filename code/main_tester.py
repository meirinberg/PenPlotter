...
def File_Draw():
    ...
    while True:
        if state.get() == 0:
            ...
            # Run rest of code
            pass
        yield None # will probably need to have its position changed
...
def Calc_HPGL():
    ...
    while True:
        ...
        args = []
        commands = []
        filename = "circle.hpgl" #replace with filetype.get()
        alpha = 3.14159*25.4
        # if filename:
...
if __name__ == "__main__":
    ...
    thetax = task_share.Queue ('L', 10, thread_protect = False, overwrite = False,
                           name = "HPGL X Output")
    ...
    class StepperMotor: 
       ...
       def convertBytesToInt(self,value):
          my_int = int.from_bytes(value[1:], byteorder='big', signed=True)
          return my_int 
